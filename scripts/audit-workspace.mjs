...
if (packageDirectories.length !== 55 || ownedNames.size !== 55) {
  fail(
    `Expected exactly 55 DoomPi packages, found ${packageDirectories.length} directories and ${ownedNames.size} names`,
  );
}
...
console.log(
  'Workspace audit passed: 51 publishable runtime packages, 1 private standalone client, 4 tooling packages, dispensable feature closure, registry-only externals, 12 materialized RMUX payloads, and 4 materialized RTK payloads.',
);

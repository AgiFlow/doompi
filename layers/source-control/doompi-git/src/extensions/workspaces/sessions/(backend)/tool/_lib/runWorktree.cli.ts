...
export default defineCliTool((context: WithRoot<PiPluginContext<Partial<GitExtensionDependencies>>, GitPiScope>) =>
  definePiTool(createRunWorktreeTool(context.root.operations)),
);
...
): ToolDefinition<typeof RunWorktreeToolSchema, RunWorktreeToolDetails> {
  const tool: ToolDefinition<typeof RunWorktreeToolSchema, RunWorktreeToolDetails> = {

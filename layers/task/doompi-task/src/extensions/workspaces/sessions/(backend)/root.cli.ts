...
import type { TaskToolDependencies } from './tool/task.cli';
...
const root = defineRoot(({ context: cordis, pi, signal: pluginSignal }: PiPluginContext) => {
  ...
  return {
    ...
    onDispose: dispose,
    onStart() {
      registerTaskCollapseShortcut(pi, taskOverlay, waitForSessionReadiness, () => active && !pluginSignal.aborted);
    },
  };
});
export type TaskPiScope = ReturnType<typeof root>['value'];
export default root;

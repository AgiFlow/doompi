...
describe('doompi-sandbox package contract', () => {
  it('is a public ESM package with an explicit publish allowlist', async () => {
    const manifest = await readManifest();

    expect(manifest.name).toBe('@agimon-ai/doompi-sandbox');
  ...
  it('keeps closed ESM, CJS, and declaration exports plus Pi discovery metadata', async () => {
    ...
    expect(Object.keys(exportsMap)).toEqual(['.', './sandbox-harness', './extensions/pi', './package.json']);

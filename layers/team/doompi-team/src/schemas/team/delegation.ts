/**
 * Subagent delegation - a cross-extension event protocol.
 *
 * Delegation lets another extension ask this runtime to run a subagent without
 * depending on its internals: the requester emits a request event and receives
 * started/update/response events back over the host event bus.
 *
 * Two generations coexist. V1 correlates purely by `requestId`. V2 adds
 * `ownerRunId` and `nodeId` so one owner can fan out to many nodes and have each
 * reply routed back, and it carries a typed `result` request.
 *
 * PROTOCOL COMPATIBILITY (do not change):
 * The event names and version numbers below are shared with doom-pi-subagents
 * and with doom-task and agiflow-pi-extension, which speak this protocol today.
 * The event names deliberately keep their `prompt-template:` prefix: that is the
 * established transport, and renaming it would silently stop delivery. Pinned by
 * contract tests.
 *
 * AVOID:
 * - Adding a required field to an existing version; add a version instead
 */
...
export type SubagentDelegationAcceptanceConfig = SubagentDelegationAcceptanceFields &
  ({ level: 'none'; reason: string } | { level?: 'auto' | 'attested' | 'checked' | 'verified'; reason?: string });
...
export interface SubagentDelegationRequest {
  ...
  artifacts?: boolean;
  /** Foreground remains the compatibility default. */
  runMode?: 'foreground' | 'detached';
  /** Optional task identity shown to active root-session teammates. */
...
export type SubagentDelegationV2Value = { kind: 'text'; text: string } | { kind: 'structured'; value: unknown };

...
export type SubagentToolDetails = Record<string, unknown>;
...
export interface SubagentToolContract {
  execute(
    ...
    onUpdate: ((result: AgentToolResult<SubagentToolDetails>) => void) | undefined,
  ...
  private async executeManagement(
    ...
    switch (params.action) {
      case SUBAGENT_ACTIONS.status: {
        ...
        if (!result.status && !result.runDir && !result.resultPath && !suspended) {
          throw new DoomTeamExpectedError(
            'run_not_found',

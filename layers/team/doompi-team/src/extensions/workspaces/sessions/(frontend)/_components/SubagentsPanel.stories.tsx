...
subagents.update('fleet-busy', (current) => ({
  ...
  runs: [
    ...
    run({
      ...
      toolCount: 9,
      error:
        'typecheck did not pass: src/extensions/workspaces/sessions/(frontend)/_components/RunControl.stories.tsx(12,7)',
    }),
  ],
}));
...
const thread = (threadId: string): ReactNode => (
  <div className="flex flex-col gap-1 px-3 py-2 text-2xs text-doom-dim">
    ...
    <span className="text-doom-text">
      read src/extensions/workspaces/sessions/(frontend)/_components/SubagentsPanel.tsx
    </span>
    <span className="text-doom-text">grep slotPropsFixture packages/core/doompi-core</span>
    <span className="text-doom-text">
      bash pnpm exec oxlint 'src/extensions/workspaces/sessions/(frontend)/_components'
    </span>
  </div>
);

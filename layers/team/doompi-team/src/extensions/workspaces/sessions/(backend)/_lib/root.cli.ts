...
const root = defineRoot(({ context: cordis, pi, signal: pluginSignal }: PiPluginContext) => {
  ...
  const readTranscriptPage: NonNullable<RegisterFleetCommandDeps['readTranscriptPage']> = (runId, request, signal) => {
  ...
  const setAgentStatus = (status: ReturnType<typeof createAgentStatus>) => {
    agentStatus = status;
    return () => {
  ...
  const runtimeService = (serviceCordis: Context) => {
    ...
    serviceCordis.inject([DOOM_UI_HUB_SERVICE], (uiContext) =>
      setAgentStatus(
        createAgentStatus(requireDoomUiHub(uiContext), {
          scheduler: pollScheduler,
          tracker: asyncJobTracker,
          environment,
        }),
...
export type TeamPiScope = Awaited<ReturnType<typeof root>>['value'];
export default root;

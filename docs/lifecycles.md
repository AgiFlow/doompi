...
**All three are real runtime mounts with their own lifetime.** The hub installs the global bundle and each workspace bundle into their own Cordis root, and the cockpit starts and stops global-scope and workspace-scope plugins per mount exactly as it does session-scope ones. What differs between the scopes is what each one selects, not whether it runs.

**Lifetimes are per mount on every host. Contributions cascade and merge on the web only.** A server facet reads exactly one declaration, `plugin[host.scope]`, and returns when that scope is absent; the loader also skips any entry that does not list the requested scope, so nothing declared under `global` ever reaches a session host. The CLI has a single scope and nothing to cascade. Only the cockpit folds global into workspace into session, a child replacing a parent's contribution when their keys match and retaining the rest, and it skips `start` while doing so precisely so that lifetimes are not merged along with contributions.
...
See [Architecture](architecture.md) for Cordis root ownership, selection transitions and child isolation, and [Composition and runtime bundling](bundling.md) for how a selection becomes the artifacts these hosts load.

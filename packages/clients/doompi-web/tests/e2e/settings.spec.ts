...
test('hides session MCP creation from a remote caller while retaining permitted metadata', async ({
  page,
  cockpit,
}) => {
  await page.route('**/api/workspaces/*/sessions/*/mcp/config', (route) =>
    route.fulfill({
      json: {
  ...
  await page.route('**/api/workspaces/*/sessions/*/mcp/clients', (route) =>
    route.fulfill({
      json: {
        clients: [
          {
            clientId: 'existing',
            name: 'Existing ChatGPT',
            redirectUri: 'https://chatgpt.com/callback',
            tokenEndpointAuthMethod: 'client_secret_post',
            createdAt: 1,
            scope: 'session',
  ...
  await page.goto(cockpit.url);
  await cockpit.session.waitForAttach();
  await page.evaluate(() => sessionStorage.setItem('doompi.channelKey', 'paired-host-key'));
  await page.goto(`${cockpit.url}/settings/remote`);
  await expect(page.getByTestId('session-mcp-remote-note')).toBeVisible();
  await expect(page.getByTestId('session-mcp-create')).toHaveCount(0);
  await expect(page.getByTestId('session-mcp-clients')).toContainText('Existing ChatGPT');

...
describe('authApi', () => {
  it('lists providers', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/plugins/doompi/providers', undefined);
  });
  ...
  it('posts a login start and reads the flow back', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/plugins/doompi/logins', {
      method: 'POST',
  ...
  it('polls, answers, and cancels a flow by id', async () => {
    ...
    expect(fetchMock).toHaveBeenLastCalledWith('/api/plugins/doompi/logins/f%201', undefined);
    await expect(answerLogin('f1', '1', 'sk')).resolves.toEqual({ flow });
    expect(fetchMock).toHaveBeenLastCalledWith('/api/plugins/doompi/logins/f1/answer', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ promptId: '1', value: 'sk' }),
    });
    await expect(cancelLogin('f1')).resolves.toEqual({ flow });
    expect(fetchMock).toHaveBeenLastCalledWith('/api/plugins/doompi/logins/f1', { method: 'DELETE' });
  });
  ...
  it('logs out a provider', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/plugins/doompi/providers/anthropic', { method: 'DELETE' });
  });

...
export interface WebBundleSyncInput {
  ...
  /** Test seam over the dynamic import of the web builder. */
...
export async function syncWebBundle(input: WebBundleSyncInput): Promise<WebBundleSyncResult> {
  ...
  if (webRoot === undefined || (!input.resolveWebRoot && !fs.existsSync(path.join(webRoot, 'package.json')))) {
  ...
  try {
    const bundler = (
      input.importBundler
        ? await input.importBundler(new URL('./bundle.mjs', import.meta.url).href)
        : await import('./bundle')
    ) as BundlerModule;
    if (typeof bundler.bundleCockpitWeb !== 'function') {
      return { status: 'failed', reason: 'Web builder exports no bundleCockpitWeb' };
    }

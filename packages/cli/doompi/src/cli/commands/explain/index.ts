...
import { type McpServerCost, priceMcpToolSchemas } from '@agimon-ai/doompi-core/mcpSchemaCost';
...
async function costExplain(context: HarnessContext): Promise<MatrixCost | undefined> {
  ...
  try {
    ...
    process.stderr.write(`[doompi] ${'explain'}: could not price the selection: ${detail}\n`);
    return undefined;
  }
}

export async function runExplain(context: HarnessContext): Promise<number> {
  const { options, resources, plugins, mcpAllowlist } = context;
  const cost = await costExplain(context);

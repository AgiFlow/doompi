import { connectDoomCordisHost, finalizeDoomCordisHost } from '@agimon-ai/doompi-core/cordisHost';
...
export async function cordisFinalizerExtension(pi: ExtensionAPI): Promise<void> {
  const connection = await connectDoomCordisHost(pi, FINALIZER_SOURCE);

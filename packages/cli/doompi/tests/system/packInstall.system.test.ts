...
const CHECKED_IN_MODES_PATH = path.join(
  REPOSITORY_ROOT,
  'packages/cli/doompi/tests/fixtures/repository/.doom/modes.yaml',
);
...
describe('consumer ownership boundaries', () => {
  ...
  it('keeps the matrix explicit instead of silently dropping standard entries', () => {
    ...
    expect(PACKAGE_MATRIX).toHaveLength(52);
    expect(standardPackageSet.size).toBe(38);
    expect(standardPackageSet).toContain('@agimon-ai/doompi-template-advanced');
    expect(standardPackageSet).toContain('@agimon-ai/doompi-template-elegant');
...
describe('DPI installed experiment runtime', () => {
  it(
    ...
    async () => {
      ...
      expect(version.code, version.stderr || version.stdout).toBe(0);
      expect(version.stdout.trim()).toBe('0.87.0');

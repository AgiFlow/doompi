...
/**
 * Truncate and pad a themed row without retaining pi-tui's injected hard reset.
 * The reset would cancel an outer background before this function's padding.
 */
export function fitStyledLine(text: string, width: number, ellipsis: string = DEFAULT_ELLIPSIS): string {
  const safeWidth = Math.max(0, width);
  const clipped = truncateToWidth(text, safeWidth, ellipsis).replaceAll(HARD_RESET, '');

...
describe('explicit remote cache guidance', () => {
  it('declares a named lazy skill rather than a local registration', () => {
    expect(skill.name).toBe('doompi-use-cache');
  ...
  it('reads the explicitly selected packaged guidance without inferred fallback', async () => {
    const expected = await readFile(new URL('../src/prompts/doompi-use-cache/SKILL.md', import.meta.url), 'utf8');

...
const { activateDoomPiEditExtension } = await import('../src/adapters/pi/extension.ts');
...
describe('standard doompi-edit extension lifecycle', () => {
  it('acquires one host lease, mounts one root fiber, and releases both once on repeated shutdown', async () => {
    const fixture = createPi();
    await activateDoomPiEditExtension(fixture.pi);

    expect(hostMocks.connect).toHaveBeenCalledOnce();
    expect(hostMocks.connect).toHaveBeenCalledWith(fixture.pi, '@agimon-ai/doompi-edit');

...
describe('doompi-read package contract', () => {
  it('retains a publishable identity, workspace Doom dependencies, and exact Pi peer versions', async () => {
    const manifest = await readManifest();
    expect(manifest.name).toBe('@agimon-ai/doompi-read');
    ...
    for (const dependency of piPeers) {
      expect(manifest.peerDependencies?.[dependency]).toBe('0.85.0');
      expect(manifest.devDependencies?.[dependency]).toBe('0.85.0');
    }
  });
  ...
  it('uses the canonical default package root and source template', async () => {
    ...
    expect(project.sourceRoot).toBe('packages/default/doompi-read/src');

...
describe('doom-mcp package boundary', () => {
  ...
  it('uses a package-local source root and declares conventional Pi discovery metadata', async () => {
    const manifest = await readManifest();
    ...
    expect(project.sourceRoot).toBe('packages/default/doompi-mcp/src');

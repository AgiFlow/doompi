...
export class McpSession {
  ...
  readonly getClientManager = (): McpClientManagerService | undefined => this.runtime?.getServices()?.clientManager;
  ...
  private trackTools(tools: readonly CatalogTool[]): void {
    for (const tool of tools) {
      ...
      if (registered === undefined) {
        ...
        this.retainedTools.set(tool.piName, tool);
        continue;
      }
  ...
  toolDefinitions(): readonly CatalogTool[] {
    return [...this.retainedTools.values()].filter(
      (tool) =>
        this.catalog.findTool(tool.piName) !== undefined &&
        !this.incompatibleNames.has(tool.piName) &&
        !this.disconnectedServers.has(tool.serverName),
    );
  }

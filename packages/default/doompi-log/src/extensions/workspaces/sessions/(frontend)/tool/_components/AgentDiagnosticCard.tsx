...
export function AgentDiagnosticCard({ toolName, output, running, isError }: ToolMessageRenderProps) {
  const lines = output
    .split('\n')
    .filter(Boolean)
  ...
  return (
    <MessageItem tone={toolTone({ running, isError })} expandable={lines.length > 12}>
      {({ expanded }) => {
        ...
        return (
          <>
            <MessageItemHeader title={toolName}>Captured agent evidence</MessageItemHeader>
            <MessageItemBody>
              <MessageLines lines={view.shown} />
              {view.hidden > 0 ? <MessageItemStatus expands>{view.hidden} more lines</MessageItemStatus> : null}
              {running ? <MessageItemStatus tone="running">Reading session telemetry</MessageItemStatus> : null}

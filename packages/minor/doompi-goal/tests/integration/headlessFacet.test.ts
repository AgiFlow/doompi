...
import { goalHeadlessFacet } from '../../src/adapters/headless/facet.ts';
...
function fixture() {
  ...
  const close = goalHeadlessFacet.apply({ get: () => host } as unknown as Context);
  return {
    execution,
    selection: () => selection,
    appendedEntries,

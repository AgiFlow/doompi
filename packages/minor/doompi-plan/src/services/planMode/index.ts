...
/** Withheld while any plan flavor is active. Restored by the surface arbiter on exit. */
const PLAN_MODE_EXCLUDED_TOOLS = new Set(['edit', 'write']);
...
export type { PlanningFlavor } from '../../services/prompts';
...
/**
 * Gate Plan-owned tools by flavor, and withhold the two file-writing tools.
 *
 * `flavor` undefined means plan mode is off, which hides plan's own tools
 * because they are registered for the whole session, and restores edit and write.
 */
...
export function createPlanModeRuntime(
  ...
  return {
              ...
              async handleAction(actionId, argumentsValue, execution) {
                ...
                if (actionId === MODE_ACTIVATE_ACTION) {
                  ...
                  await queueTransition(() =>
                    activateFlavor(execution.context, flavor, PLAN_MODE_TRIGGER_LEADER, false),
                  );
              ...
              onError: (error, actionName) => {
                ...
                if (activeContext?.hasUI) {
                  activeContext.ui.notify(
                    `Leader action ${actionName} failed. Plan mode remains read-only.`,
                    WARNING_STYLE,
                  );
                }
                void telemetry.recordError(PLAN_EVENT.modeEnabled, error, {
    ...
    onStop: shutdownPlanRuntime,
    onDispose: shutdownPlanRuntime,
    tools: [
      {
        ...
        promptSnippet:
          'Record the issue and any relevant reproduction, logs, traces, process output, or browser evidence',
        parameters: {
          type: 'object',
          properties: {
            issue: { type: 'string' },
      ...
      },
      {
        ...
        async execute(toolCallId, _params, signal, onUpdate, ctx) {
          ...
          try {
            ...
            const resolvedPlansDirectory = await waitForAbortable(
              () => fs.promises.realpath(plansDirectory),
              writeSignal,
            );
            if (directoryStats.isSymbolicLink() || !directoryStats.isDirectory()) {
              recordRefusal(
                'unsafe_plans_directory',
                'Refused to write the plan because the configured path is unsafe.',
              );
              return {
            ...
            try {
              ...
              if (
                ...
                'code' in error &&
                ['EISDIR', 'ELOOP', 'ENXIO'].includes(String(error.code))
              ) {
                recordRefusal('invalid_destination', INVALID_PLAN_DESTINATION);
                return {
                  content: [{ type: 'text', text: INVALID_PLAN_DESTINATION }],
    ...
    events: {
      ...
      session_start: async (_event, ctx) => {
        ...
        const state = entries
          .flatMap((entry) =>
            entry.type === 'custom' && entry.customType === PLAN_MODE_ENTRY
              ? [parsePersistedPlanState(entry.data)]
              : [],
          )

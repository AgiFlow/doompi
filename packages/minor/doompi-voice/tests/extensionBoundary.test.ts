...
describe('doom voice extension boundaries', () => {
  it('exposes one standard Pi factory with typed host integrations folded into it', async () => {
    ...
    const standardFactory = await readSource('src/services/voiceControllerPlugin.ts');
    const implementation = await readSource('src/services/voiceController.ts');
  ...
  it('keeps the standalone narration tool behind the controller playback boundary', async () => {
    const narrationTool = await readSource('src/services/narrationToolController.ts');

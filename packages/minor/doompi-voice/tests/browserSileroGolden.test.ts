...
import { SILERO_STATE_SAMPLES, SileroVadFrames } from '../src/extensions/workspaces/sessions/(frontend)/lifecycle/_lib/sileroVadFrames';
...
describe('browser Silero pinned-model golden PCM', () => {
  it('detects the repository speech fixture and rejects equal-length silence', async () => {
    ...
    const model = fs.readFileSync(new URL('../src/extensions/workspaces/sessions/(frontend)/lifecycle/_lib/models/silero_vad_v6.2.1.onnx', import.meta.url));

...
export interface VoiceToolFacadeRegistration extends PiToolCollection {
  /**
...
export function createVoiceToolFacades(
  sessionProvider: VoiceToolSessionProvider,
  ...
  let describe = definePiTool(createDescribeTool(live, waitUntilReady, digest));
  const use = definePiTool(createUseTool(live, waitUntilReady));
  const listeners = new Set<() => void>();
  return {
    snapshot: () => (disposed ? [] : [describe, use]),
    subscribe(listener) {
    ...
    refresh() {
      ...
      digest = next;
      describe = definePiTool(createDescribeTool(live, waitUntilReady, digest));

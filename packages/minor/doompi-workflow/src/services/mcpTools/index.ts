...
export const WORKFLOW_MCP_TOOLS_SERVICE = 'doom/workflow-mcp-tools';
...
export function mountMcpTools(tools: readonly DoomHeadlessTool[]) {
  return (context: Context): void => {
    context.plugin((providerContext) => {
      providerContext.provide(WORKFLOW_MCP_TOOLS_SERVICE, {
...
export function bindMcpTool(context: DoomMcpPluginContext, name: string): DoomHeadlessTool {
  const tool = context.services.get<McpToolCatalog>(WORKFLOW_MCP_TOOLS_SERVICE)?.get(name);
  if (!tool) throw new Error(`Workflow tool is unavailable: ${name}`);
  return {
    ...tool,

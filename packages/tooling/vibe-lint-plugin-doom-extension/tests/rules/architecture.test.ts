...
describe('Doom deterministic architecture rules', () => {
  ...
  // src/web is retired. Browser code colocates with the routed file that renders
  // it, so web is no longer a canonical root and the folder rule reports it.
  it('rejects src/web as a canonical root', () => {
    ...
    const report = doomFolderLayout.check?.(panel, root, boundaryContext());
    expect(report).toContain('Unknown root "web"');
    expect(report).toContain('(frontend)/_components');
  });

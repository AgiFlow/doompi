...
describe('Pi extension default factory rule', () => {
  ...
  it('accepts direct default function declarations', () => {
    declareEntry();
    const entry = write('src/extensions/pi.ts', 'export default function activate(): void {}');
  ...
  it('accepts imported and forwarded default factories', () => {
    ...
    const imported = write(
      'src/extensions/pi.ts',
      ["import activate from './activate';", 'export default activate;'].join('\n'),
    );
    expect(piExtensionDefaultFactory.check?.(imported, root, boundaryContext())).toBeNull();

    const forwarded = write('src/extensions/pi.ts', "export { activate as default } from './activate';");
  ...
  it('accepts factory creator calls and rejects non-callable defaults', () => {
    ...
    const call = write(
      'src/extensions/pi.ts',
      ["import { createExtension } from './create';", 'export default createExtension();'].join('\n'),
    );
    expect(piExtensionDefaultFactory.check?.(call, root, boundaryContext())).toBeNull();

    const value = write('src/extensions/pi.ts', "export default 'not-a-factory';");
  ...
  it('ignores helpers, unsupported files, missing files, and paths outside the config root', () => {
    const helper = write('src/extensions/helper.ts', 'export const helper = true;');
    const json = write('src/extensions/pi.json', '{}');

...
export function loadHarnessState(
  ...
  if (cacheable && harnessCache.loaded && cacheCurrent(harnessCache.loaded, environment)) return harnessCache.loaded;
  ...
  if (cacheable) harnessCache.loaded = result;
  return result;
}
...
export function restoreHarnessStateSnapshot(
  ...
  if (environment === process.env) {
    harnessCache.loaded = snapshot.filePath
      ? {
...
export function createHarnessSession(
  ...
  if (!options.unclaimed && options.environment === process.env)
    harnessCache.loaded = { filePath, owned: true, state, mtimeMs: fileMtime(filePath) };
  return filePath;
}
...
export function updateHarnessState(
  ...
  if (environment === process.env) harnessCache.loaded = { filePath, owned: true, state, mtimeMs: fileMtime(filePath) };
...
export function disposeHarnessState(environment: NodeJS.ProcessEnv = process.env): void {
  const current = harnessCache.loaded;
  resetHarnessStore();

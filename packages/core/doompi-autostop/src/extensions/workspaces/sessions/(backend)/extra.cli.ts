...
export default ({ options }: PiPluginContext<AutoStopDelays>) => {
  ...
  return {
    ...
    events: {
      ...
      agent_settled: (
        _event: Parameters<NonNullable<PiEventHandlers['agent_settled']>>[0],
        context: Parameters<NonNullable<PiEventHandlers['agent_settled']>>[1],

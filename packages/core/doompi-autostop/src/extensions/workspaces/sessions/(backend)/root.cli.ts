...
const root = defineRoot(({ options }: PiPluginContext<AutoStopDelays>) => {
  const backgroundWork = createBackgroundWorkGate();
  const watch = createIdleShutdown(options ?? DEFAULT_AUTO_STOP_DELAYS, (context) => backgroundWork.hasActiveWork(context.sessionManager.getSessionId()));
  return { value: watch, services: [backgroundWork.plugin], onDispose: watch.cancel };
});
export type AutoStopPiScope = ReturnType<typeof root>['value'];
export default root;

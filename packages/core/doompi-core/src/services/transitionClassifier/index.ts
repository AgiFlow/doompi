...
function launcherNeedsRelaunch(
  extensionLayers: TransitionClassifierContext['extensionLayers'],
  previous: TransitionSelectionSnapshot,
  candidate: TransitionSelectionSnapshot,
): boolean {
  ...
  return !sameStrings(extensionLayers(previous.layers), extensionLayers(candidate.layers));
}
...
function majorModeDisposition(
  ...
  return launcherNeedsRelaunch(extensionLayers, previous, candidate)
    ? {
...
export function classifyTransition(
  ...
  switch (request.target.axis) {
    case 'live':
      return plan(request, 'live', previous, copySnapshot(previous), 'transition.live.capability');
    case 'domains': {
    ...
    case 'major-mode': {
      const candidateLayers = context.resolveLayers(request.target.majorMode);

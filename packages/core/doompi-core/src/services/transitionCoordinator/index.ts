import type {
  ...
  TransitionOutcome,
} from '../../exports/transition';
import { classifyTransition, type TransitionClassifierContext } from '../transitionClassifier';

const LIVE_AXIS = 'live';
...
export function createDoomTransitionCoordinator(options: TransitionCoordinatorOptions): DoomTransitionCoordinator {
  ...
  const requestDiagnostic = (request: DoomTransitionRequest): TransitionDiagnosticCode | undefined => {
    ...
    if (request.target.axis !== LIVE_AXIS && operationIds.has(request.operationId)) {
  ...
  const executeLive = async (
    ...
    if (request.target.axis !== LIVE_AXIS) {
      return result(transitionPlan, 'rejected', EXECUTION_DIAGNOSTIC);
    }
    if (!execution) return result(transitionPlan, 'rejected', 'transition.rejected.unavailable');
  ...
  return {
    ...
    async execute(request, execution) {
      ...
      if (request.target.axis !== LIVE_AXIS) operationIds.add(request.operationId);
      ...
      if (request.target.axis === LIVE_AXIS) {
        ...
        return executeLive(request, transitionPlan, execution, expectedGeneration);
      }

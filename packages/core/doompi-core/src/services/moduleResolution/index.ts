 ...
 */
export function packageEntry(name: string, origin: string = import.meta.url): string {
  const entry = consumerPackageEntry(name, path.dirname(fileURLToPath(origin)));
  if (!entry) throw new Error(`Cannot resolve package entry ${name} from ${origin}`);
  return fs.realpathSync(entry);
}

/** Same as packageEntry, but returns undefined when the package is absent. */
export function optionalPackageEntry(name: string, origin: string = import.meta.url): string | undefined {
  try {
    return packageEntry(name, origin);
  } catch {
    return undefined;
  }
}
...
export function packageEntries(specifier: string, origin: string = import.meta.url): string[] {
  const { name, subpath } = splitPackageSpecifier(specifier);
  if (subpath !== '.') return [packageEntry(specifier, origin)];

  const packageRoot = findInstalledPackage(name, path.dirname(fileURLToPath(origin)));
...
export function optionalPackageEntries(specifier: string, origin: string = import.meta.url): string[] | undefined {
  try {
    return packageEntries(specifier, origin);
  } catch {
    return undefined;
  }
}
...
export function consumerPackageEntry(specifier: string, consumerRoot: string): string | undefined {
  ...
  if (subpath === '.') return localEntry(packageRoot, consumerRoot) ?? extensionIndexEntry(packageRoot);

...
export class HeadlessHost extends Service<DoomHeadlessHostService> implements DoomHeadlessHostService {
  ...
  private matches(when?: DoomHeadlessCondition): boolean {
    return (
      Object.entries(when?.state ?? {}).every(([key, value]) => this.applying.state?.[key]?.includes(value)) &&
      (when?.domain === undefined || this.applying.domains.includes(when.domain))
    );
  }
  private selected<T extends { when?: DoomHeadlessCondition }>(entries: readonly Owned<T>[]): readonly Owned<T>[] {
    return entries.filter(({ value }) => this.matches(value.when));
  }

  assertActive(source?: string): void {
    this.assertReady();
    if (source !== undefined && !this.activeSources.has(source))
      throw new Error(`Contribution owner '${source}' is inactive`);
  }
  ...
  changeSelection(change: DoomHeadlessSelectionChange): Promise<void> {
    ...
    return this.select({ state: { ...this.requested.state, [change.key]: [...change.values] } });
  }
  ...
  private async refreshChangedSelection(
    ...
    const stateChanged = JSON.stringify(previous.state ?? {}) !== JSON.stringify(selection.state ?? {});

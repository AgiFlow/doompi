...
import { exportV4ToV3 } from '../adapters/serialization/v3Export.ts';
import type { HistoryOwnership } from '../adapters/serialization/historyImport.ts';
import { historyExportHelp } from './cli/help.ts';
import { wantsHelp } from './cli/router.ts';

const COMMAND = 'history-export';
type ValueOption =
  | '--source'
  | '--source-path'
  | '--destination'
...
export interface HistoryExportCommandOptions {
...
export class HistoryExportCommand {
  readonly name = COMMAND;
  private readonly owner: HistoryOwnership;

  constructor(options: HistoryExportCommandOptions = {}) {
    this.owner = options.owner ?? createHistoryOwnership();
  }

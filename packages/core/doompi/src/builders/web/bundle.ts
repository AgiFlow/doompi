 ...
 * the host shell and each plugin's shipped web source into one browser bundle.
 * The roots file beside
...
export async function bundleCockpitWeb(options: BundleCockpitWebOptions): Promise<BundleCockpitWebResult> {
  ...
  const generated = writeSyncWebPluginModules(plugins, generatedDir, hostRoot);

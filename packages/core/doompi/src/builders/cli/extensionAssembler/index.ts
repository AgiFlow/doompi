...
export const LAYER_RESOLVERS: ExtensionLayerResolvers = {
  ownEntry,
  packageEntry: (specifier) => resolvePackageEntry(specifier, import.meta.url),
  optionalPackageEntry: (specifier) => resolveOptionalPackageEntry(specifier, import.meta.url),
  packageEntries: (specifier) => resolvePackageEntries(specifier, import.meta.url),
  optionalPackageEntries: (specifier) => resolveOptionalPackageEntries(specifier, import.meta.url),
  localEntry,
  localEntries,
  localPackageEntries,
...
export function createLayerResolvers(consumerRoot?: string): ExtensionLayerResolvers {
  ...
  const fallbackEntry = (name: string): string | undefined =>
    FIXED_CORE_PACKAGES.has(packageName(name)) ? resolveOptionalPackageEntry(name, import.meta.url) : undefined;
  const fallbackEntries = (name: string): string[] | undefined =>
    FIXED_CORE_PACKAGES.has(packageName(name)) ? resolveOptionalPackageEntries(name, import.meta.url) : undefined;

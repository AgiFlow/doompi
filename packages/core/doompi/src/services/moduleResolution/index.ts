...
/**
 * Resolves one of this package's own extensions in src/extensions.
 *
...
export function ownEntry(name: string): string {
  ...
  // This file sits in src/services/moduleResolution, so the source root is two directories up.
  const sourceRoot = path.dirname(path.dirname(path.dirname(fileURLToPath(import.meta.url))));
  const entryName = extension === 'ts' ? name : name.replace(/([a-z0-9])([A-Z])/g, '$1-$2').toLowerCase();

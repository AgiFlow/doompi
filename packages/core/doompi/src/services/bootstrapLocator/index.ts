...
import { resolveDoomConfigurationRoot } from '../repository';
import { readSyncRegistration } from '../syncRegistration';
import { readSyncState } from '../syncState';
import { BUNDLED_PRECOMPILE_STRATEGY, PRECOMPILE_STATE_VERSION } from '../syncStateContract';
...
function packagedDoomEntry(moduleUrl: string = import.meta.url): string {
  ...
  const sourceRoot = path.dirname(path.dirname(path.dirname(fileURLToPath(moduleUrl))));
  const entryName = extension === 'ts' ? 'composedPi' : 'composed-pi';
  return path.join(sourceRoot, 'extensions', `${entryName}.${extension}`);
}

...
export class CliApp {
  ...
  private async getTelemetry(): Promise<HarnessTelemetry> {
    ...
    const { createHarnessTelemetry } = await import('../services/logSinkTelemetry');
  ...
  async selectCommand(options: HarnessOptions): Promise<BaseCommand> {
    if (options.emitMcp) {
      const { EmitMcpCommand } = await import('./emitMcpCommand');
      return new EmitMcpCommand();
    }
    if (options.explain) {
      const { ExplainCommand } = await import('./explainCommand');
      return new ExplainCommand();
    }
    if (options.sandbox) {
      const { SandboxLaunchCommand } = await import('./sandboxLaunchCommand');
      return new SandboxLaunchCommand();
    }
    const { LaunchCommand } = await import('./launchCommand');
  ...
  async runHarness(options: HarnessOptions): Promise<number> {
    ...
    const [{ buildHarnessContext }, { ensureLayerPackages }, { HARNESS_EVENT }] = await Promise.all([
      import('../services/harnessContext'),
      import('../services/layerPackageInstaller'),
      import('../services/logSinkTelemetry'),
    ]);
  ...
  async run(args: string[]): Promise<number> {
    ...
    if (args[0] === 'sync') {
      const [{ SyncPipeline }, telemetry] = await Promise.all([import('./syncPipeline'), this.getTelemetry()]);
    ...
    if (args[0] === 'compat') {
      const [{ CompatibilityCommand }, telemetry] = await Promise.all([
        import('./compatibilityCommand'),
        this.getTelemetry(),
      ]);
      return new CompatibilityCommand(telemetry).execute(args);
    }
    if (args[0] === 'history-export') {
      const { HistoryExportCommand } = await import('./historyExportCommand');
      return new HistoryExportCommand().execute(args);
    }
    if (args[0] === 'history-import') {
      const { HistoryImportCommand } = await import('./historyImportCommand');

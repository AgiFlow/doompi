...
describe('machineApi', () => {
  it('requires a global mount and closes the provider service', async () => {
    ...
    expect(await response(handler, '/providers')).toEqual({ status: 503, body: { error: 'Provider mount is closed.' } });
    expect(auth.listProviders).toHaveBeenCalledOnce();
  });

  it('maps provider logout outcomes to HTTP statuses', async () => {
    const handler = machineApi.start(context);
    expect(await response(handler, '/providers/test%20provider', 'DELETE')).toEqual({ status: 200, body: { ok: true } });
    ...
    expect(await response(handler, '/providers/missing', 'DELETE')).toEqual({ status: 404, body: { error: 'unknown' } });
    ...
    expect(await response(handler, '/providers/test', 'GET')).toEqual({ status: 404, body: { error: 'Provider route not found.' } });
    handler.close();
  });
  ...
  it('reads, answers, and cancels a flow only while it exists', async () => {
    const handler = machineApi.start(context);
    expect(await response(handler, '/logins/flow')).toEqual({ status: 200, body: { flow: { id: 'flow', status: 'running' } } });
    ...
    expect(await response(handler, '/logins/missing')).toEqual({ status: 404, body: { error: 'Login flow not found.' } });
    handler.close();
  });
  ...
  it('reports invalid JSON and provider failures without leaking exceptions', async () => {
    ...
    expect(await response(handler, '/elsewhere')).toEqual({ status: 404, body: { error: 'Provider route not found.' } });
    handler.close();
  });
});

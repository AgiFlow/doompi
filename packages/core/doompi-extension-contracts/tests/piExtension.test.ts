...
import { definePiExtension, type PiEventHandlers } from '../src/controllers/piExtension';
import { createPiTestHost } from '../src/controllers/piTestHost';
import { createDoomHelpService, DOOM_HELP_SERVICE } from '../src/schemas/help';
import { DOOM_MINOR_MODE_CATALOG_SERVICE, type MinorModeCatalogService } from '../src/schemas/mode';
import { defineMinorMode } from '../src/schemas/minorModeFactory';
import { defineCommand, defineTool } from '../src/schemas/pluginContributions';
...
describe('declarative Pi extension', () => {
  ...
  it('attaches a minor mode owner to replacement catalogs and detaches on shutdown', async () => {
    ...
    const onStart = vi.fn();
    const extension = definePiExtension({ name: '@test/mode', minorModes: [owner], onStart });
    const plugin = root.plugin((context) => extension.install(context, host.pi));
    await plugin;

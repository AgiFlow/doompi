import { defineServerPlugin } from '../src/controllers/serverPlugin';
import type { MinorModeOwner } from '../src/schemas/minorModeFactory';
import { Type } from 'typebox';
import { defineTool, defineCommand } from '../src/schemas/pluginContributions';
import { DOOM_HEADLESS_HOST_SERVICE, type DoomHeadlessHostService } from '../src/schemas/headless';
...
describe('declarative server contributions', () => {
  ...
  it('owns minor mode handles and detaches the owner on shutdown', async () => {
    ...
    const registerMinorMode = vi.fn(() => handle);
    context.provide(DOOM_HEADLESS_HOST_SERVICE, { registerMinorMode } as unknown as DoomHeadlessHostService);
    const mode = {
      definition: {},
      attach: vi.fn(),
      detach() {
    ...
    const dispose = await defineServerPlugin({ name: 'mode', session: { minorModes: [mode] } }).apply(context);

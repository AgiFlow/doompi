...
export interface CreateDoomServerHostOptions {
  ...
  /** Shared exact-scope dispatch table, owned by the server composition. */
...
export function createDoomServerHost(options: CreateDoomServerHostOptions): DoomServerHost {
  ...
  const registerMethod: DoomServerHostService['registerMethod'] = (definition, handler) => {
    if (disposed || !options.pluginRegistry) {
      notice(
        `plugin method '${definition.service}.${definition.method}' is skipped: this host has no plugin dispatcher.`,
      );
      return noop;
    }
    const mount =
      scope === 'global'
        ...
        : scope === 'workspace' && context.workspaceId
          ? { scope: 'workspace' as const, workspaceId: context.workspaceId }
          : scope === 'session' && context.sessionId
            ? { scope: 'session' as const, sessionId: context.sessionId }
            : undefined;
    if (!mount) {
      notice(`plugin method '${definition.service}.${definition.method}' is skipped: this host has no mount identity.`);
      return noop;
    }
    try {
      const release = options.pluginRegistry.register(mount, definition, handler);
      let released = false;
      const dispose = () => {
        if (released) return;
        released = true;
        methods.delete(dispose);
        release();
      };
      methods.add(dispose);
      return { mounted: true, dispose };
    } catch (error) {
      notice(`plugin method '${definition.service}.${definition.method}' did not mount (${String(error)}).`);
      return noop;
    }
  };
  ...
  return {
    ...
    dispose: () => {
      if (disposed) return;
      disposed = true;
      for (const release of [...methods].reverse()) release();

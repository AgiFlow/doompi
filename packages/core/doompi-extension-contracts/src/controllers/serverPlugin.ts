    ...
    async apply(context) {
      ...
      await lifecycle.mount(
        ...
        async (scope) => {
          ...
          for (const mode of session.minorModes ?? []) {
            if ('definition' in mode) {
              const handle = agent.registerMinorMode(mode.definition);
              own(handle);
              lifecycle.own(() => mode.detach());
              mode.attach(handle);
            } else own(agent.registerMinorMode(mode));
          }

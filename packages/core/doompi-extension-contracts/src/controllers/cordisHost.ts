...
import { createDoomContextContributionsService } from '../services/contextContributions';
import { createDoomToolOverridesService } from '../services/toolOverrides';
import { createDoomToolSurface } from '../services/toolSurface';
...
export function requireDoomCordisSession(context: Context): DoomCordisSessionService {
  const session = context.get(DOOM_CORDIS_SESSION_SERVICE) as DoomCordisSessionService | undefined;
  if (!session) throw new Error('Doom Cordis session service is unavailable.');
  return session;
}

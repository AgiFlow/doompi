...
class ScopedDoomServerHost extends Service<DoomServerHostService> implements DoomServerHostService {
  ...
  registerMethod: DoomServerHostService['registerMethod'] = (definition, handler) => {
    const registration = this.host.registerMethod(definition, handler);
    if (registration.mounted !== true) {
      registration.dispose();
      throw new Error(`plugin method '${definition.service}.${definition.method}' did not mount.`);
    }
    this.ctx.effect(() => () => registration.dispose(), 'server facet method registration');
    return registration;
  };

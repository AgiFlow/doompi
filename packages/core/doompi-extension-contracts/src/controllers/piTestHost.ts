...
import { connectDoomCordisHost, installDoomCordisHost, type DoomCordisHostConnection } from './cordisHost';
...
export function createPiTestHost(options: PiTestHostOptions = {}): PiTestHost {
  ...
  return {
    ...
    tool: (name) => tools.findLast((candidate) => candidate.name === name),

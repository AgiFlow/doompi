...
import { DOOM_MINOR_MODE_CATALOG_SERVICE, createMinorModeCatalogClient, requireMinorModeCatalog } from '@agimon-ai/doompi-minor-mode';
...
const root = defineRoot(({ context, pi, options, runtime }: PiPluginContext<UiTelemetry>) => {
  ...
  const service = (owner: PiPluginContext['context']) => {
    ...
    owner.inject([DOOM_MINOR_MODE_CATALOG_SERVICE], (modeContext) => {
      ...
      const unsubscribe = catalog.subscribe(update); update();
      return () => { unsubscribe(); ui.uiState.setModes([]); };
    });
  };
  return { value: { ui, tools, runtime }, services: [service], onStop: () => ui.dispose(), onDispose: () => ui.dispose() };
});
export type UiPiScope = ReturnType<typeof root>['value'];
export default root;

...
export default ({ context, pi, options, runtime }: PiPluginContext<UiTelemetry>) => {
  ...
  return {
    services: [
      (owner: PiPluginContext['context']) => {
        owner.provide(DOOM_UI_HUB_SERVICE, ui.hub);
        owner.inject([DOOM_MINOR_MODE_CATALOG_SERVICE], (modeContext: PiPluginContext['context']) => {
